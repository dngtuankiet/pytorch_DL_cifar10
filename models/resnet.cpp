...
namespace model {
    namespace resnet {
        ResNetImpl::ResNetImpl(const char* block,
            ...
            conv1_(register_module("conv1", torch::nn::Conv2d(torch::nn::Conv2dOptions(3, in_planes_, 7).stride(2).padding(3).bias(false)))),
            bn1_(register_module("bn1", torch::nn::BatchNorm2d(in_planes_))),
        ...
        torch::nn::Sequential ResNetImpl::MakeLayer(int64_t planes, int64_t blocks, int64_t stride) {
            ...
            if (stride != 1 || in_planes_ != planes * expansion_) {
                downsample = torch::nn::Sequential(
                    ...
                    torch::nn::BatchNorm2d(planes * expansion_)
                );
            }
        ...
        BottleneckImpl::BottleneckImpl(int64_t in_planes,
            ...
            bn1_(register_module("bn1", torch::nn::BatchNorm2d(width_))),
            conv2_(register_module("conv2", Conv3x3(width_, width_, stride, groups))),
            bn2_(register_module("bn2", torch::nn::BatchNorm2d(width_))),
            conv3_(register_module("conv3", Conv1x1(width_, out_planes* BottleneckImpl::kExpansion))),
            bn3_(register_module("bn3", torch::nn::BatchNorm2d(out_planes* BottleneckImpl::kExpansion))),
        ...
        BasicBlockImpl::BasicBlockImpl(int64_t in_planes,
            ...
            bn1_(register_module("bn1", torch::nn::BatchNorm2d(out_planes))),
            conv2_(register_module("conv2", Conv3x3(out_planes, out_planes))),
            bn2_(register_module("bn2", torch::nn::BatchNorm2d(out_planes))),
            downsample_(register_module("downsample", downsample)),
            stride_(stride)
        {};
