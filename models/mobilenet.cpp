    ...
    namespace mobilenet {
        MobileNetV2Impl::MobileNetV2Impl(int64_t num_classes, double width_mult) {
            ...
            vector<tuple<ExpandRatio, Channel, NumBlocks, Stride>> inverted_residual_setting{
              make_tuple(1, 16, 1, 1),
              make_tuple(6, 24, 2, 2),//stride to 1
              make_tuple(6, 32, 3, 2),
              make_tuple(6, 64, 4, 2),
              make_tuple(6, 96, 3, 1),
            ...
            body->push_back(ConvBNReLU(3, input_channel, 3, 2)); //stride to 1
            ...
            features_ = register_module("features_", body);
            classifier_ = register_module("classifier_", torch::nn::Linear(last_channel, num_classes));
            initialize();
        };
        ...
        InvertedResidualImpl::InvertedResidualImpl(int64_t in_planes, int64_t out_planes, int64_t stride, int64_t expand_ratio) {
            ...
            body->push_back(torch::nn::Conv2d(torch::nn::Conv2dOptions(hidden_dim, out_planes, 1).stride(1).padding(0).bias(false)));
            body->push_back(torch::nn::BatchNorm2d(out_planes));
        ...
        ConvBNReLUImpl::ConvBNReLUImpl(int64_t in_planes,
            ...
            : conv_(register_module("conv", torch::nn::Conv2d(
                ...
                .bias(false)))),
            bn_(register_module("bn", torch::nn::BatchNorm2d(out_planes)))
        {};
